# app.py — single-page, retry-hardened, overwrite-safe, robust for 5k+ annotations
...
def read_text_from_drive(drive, file_id: str) -> str:
    try:
        ...
        if cached is not None:
            # keep UI responsive on transient errors
            return cached
        raise
...
# ---- Micro buffer for logs (reduces write calls ~6x) ----
...
def _flush_logs_if_needed(cfg, force: bool = False):
    ...
    try:
        ...
        # non-fatal, keep buffer for next try
...
with left:
    ...
    # Resolve Drive IDs (fast via cached folder index)
